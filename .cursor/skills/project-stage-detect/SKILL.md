...
if it exists, use its value (explicit override from the `gate-check` skill). Otherwise,
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run the `map-systems` skill?"
...
Use template: `.cursor/docs/studio/templates/project-stage-report.md`
...
If user provided a role argument (e.g., the `project-stage-detect` skill (programmer)):
...
```text
# General project analysis — run the `project-stage-detect` skill with no extra args.

# Programmer-focused analysis — run the `project-stage-detect` skill and ask for a programmer lens.

# Designer-focused analysis — run the `project-stage-detect` skill and ask for a designer lens.
```
...
- **Concept exists but no systems index?** → the `map-systems` skill to decompose into systems
- **Missing design docs?** → the `reverse-document` skill (design src/[system])
- **Missing architecture docs?** → the `architecture-decision` skill or the `reverse-document` skill (architecture)
- **Prototypes need documentation?** → the `reverse-document` skill (concept prototypes/[name])
- **No sprint plan?** → the `sprint-plan` skill
- **Approaching milestone?** → the `milestone-review` skill
