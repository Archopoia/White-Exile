...
4. **Generate the game concept document** using the template at
   `.cursor/docs/studio/templates/game-concept.md`. Fill in ALL sections from the
...
6. **Suggest next steps** (in this order — this is the professional studio
   ...
   - "Author per-system GDDs with the `design-system` skill — guided, section-by-section GDD writing"
   - "Prototype the core loop with the `prototype` skill ([core-mechanic])"
   - "Playtest the prototype with the `playtest-report` skill to validate the hypothesis"
