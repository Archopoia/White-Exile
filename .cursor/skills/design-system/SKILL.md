...
> "Usage: the `design-system` skill (<system-name>) — e.g., the `design-system` skill (combat-system)
> Run the `map-systems` skill first to create the systems index, then use this skill
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run the `brainstorm` skill first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run the `map-systems` skill first to map your systems."
...
Use the template structure from `.cursor/docs/studio/templates/game-design-document.md`:
...
Use `AskUserQuestion`:
- "Run the `design-review` skill now to validate the GDD?"
...
- "What's next?"
  - Options:
    ...
    - "Run the `gate-check` skill" — if enough MVP systems are designed
