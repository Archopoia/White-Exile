...
- **No argument**: the `map-systems` skill — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: the `map-systems` skill (next) — Pick the highest-priority undesigned system
  from the index and hand off to the `design-system` skill (Phase 6).
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run the `brainstorm` skill first
...
Using the template at `.cursor/docs/studio/templates/systems-index.md`, populate the
...
## 7. Phase 6: Design Individual Systems (Handoff to `design-system` skill)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes the `map-systems` skill with a specific system name
- The user invokes the `map-systems` skill with the argument `next`
...
### Step 6b: Hand Off to `design-system` skill

Once a system is selected, invoke the `design-system` skill for that system.
...
- Runs the `design-review` skill when complete
- Updates the systems index

**Do not duplicate the `design-system` skill workflow here.** This skill owns the systems
*index*; the `design-system` skill owns individual system *GDDs*.

### Step 6c: Loop or Stop

After the `design-system` skill completes, use `AskUserQuestion`:
...
- "Run the `design-system` skill ([system-name]) to write the next system's GDD"
- "Run the `design-review` skill ([path]) on each completed GDD to validate quality"
- "Run the `gate-check` skill (pre-production) to check if you're ready to start building"
- "Prototype the highest-risk system with the `prototype` skill ([system])"
- "Plan the first implementation sprint with the `sprint-plan` skill (new)"
...
2. **AskUserQuestion** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to the `design-system` skill
...
5. **Handoff**: Individual GDD authoring is owned by the `design-system` skill, which handles
