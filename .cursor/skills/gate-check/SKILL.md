...
**Distinct from the `project-stage-detect` skill**: That skill is diagnostic ("where are we?").
...
- **With argument**: the `gate-check` skill (production) — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  the `project-stage-detect` skill, then validate the NEXT phase transition
...
**Quality Checks:**
- [ ] Game concept has been reviewed (the `design-review` skill verdict not MAJOR REVISION NEEDED)
...
- [ ] At least 1 playtest report (or the `playtest-report` skill has been run)
...
- [ ] QA test plan exists
- [ ] Balance data has been reviewed (the `balance-check` skill run)
- [ ] Release checklist completed (the `release-checklist` skill or the `launch-checklist` skill run)
...
- For performance checks: `Read` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent output from the `perf-profile` skill
...
- "Performance profiling data isn't available. Would you like to run the `perf-profile` skill?"
...
### Blockers
1. **No Architecture Decision Records** — Run the `architecture-decision` skill to create one
...
- **No game concept?** → the `brainstorm` skill to create one
- **No systems index?** → the `map-systems` skill to decompose the concept into systems
- **Missing design docs?** → the `reverse-document` skill or delegate to `game-designer`
- **Missing ADRs?** → the `architecture-decision` skill
- **Tests failing?** → delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** → the `playtest-report` skill
- **Performance unknown?** → the `perf-profile` skill
- **Not localized?** → the `localize` skill
- **Ready for release?** → the `launch-checklist` skill
