...
**Format**: Run the `reverse-document` skill with a **type** (`design` | `architecture` | `concept`) and a **path** (folder or file under this repo).
...
**Examples** (natural-language invocations):

- “Run `reverse-document` with type `design` and path `src/...`”
- “Run `reverse-document` with type `architecture` and path `src/core/...`”
- “Run `reverse-document` with type `concept` and path `prototypes/...`”
...
1. Run the `balance-check` skill on combat formulas (exponential scaling concern)
...
```
User: Run the `reverse-document` skill — type design, path src/gameplay/combat
...
Agent: ✅ Written to design/gdd/combat-system.md
       ...
       - Run the `balance-check` skill to validate progression curve
