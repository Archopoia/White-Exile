...
- **Esc** opens a compact tabbed Session dialog ([`options.ts`](../apps/client/src/options.ts)): identity, graphics (quality, labels, fog/fill/exposure/sky haze, **torch reach** multiplier `rtRoom.torchReachMul` for client-only PointLight distance on all player flames, **dune height** → `roomSettingsPatch.duneHeightScale`), and Help. The room note is HUD-only. In-world labels can still surface **R** / **F** when you are close to a stranded follower (inside your light) or an inactive ruin.
