...
- **NPR post stack** ([`nprPost.ts`](../apps/client/src/nprPost.ts) + [`nprSettings.ts`](../apps/client/src/nprSettings.ts)): when the user enables it from ESC → NPR, `RoomScene.render` swaps from a single `renderer.render` to a depth+normal prepass followed by a fullscreen composite. The composite (a Three.js port of `tutelary.rendering_npr_stack`'s composite) does, in order: cel quantise (stepped shadows — more steps = harder bands), Kuwahara oil (Rembrandt-style smear), depth-edge mist (painterly halo), depth+normal Sobel ink (toon / Moebius outline), and crosshatch (classic diagonal / vertical / horizontal screen-space stripes plus optional halftone dots; `hatchCrossSteps` bins luma between dark/light thresholds — wiggle applies to Sobel outlines only, not hatch). Style presets (`toon`, `moebius`, `rembrandt`, `painterly_toon`, `custom`) preload knob bundles; every knob is also individually tweakable in the menu and persisted as `rtRoom.npr` in `localStorage`. The sky dome is hidden during the normal prepass so its sphere doesn't generate phantom Sobel edges.
